...
async def help(interaction):
    await interaction.response.send_message(
        ...
        "Sends the current version of John Bot.\n/new: Displays what is new with John Bot.", ephemeral=True)
...
async def version(interaction):
    await interaction.response.send_message("Currently Running Version 0.1.5 of John Bot.",
                                            ephemeral=True)
...
async def new(interaction):
    await interaction.response.send_message("What's new in 0.1.5:\n\n-Fixed a bug where the /feedback command did not "
                                            "exist.", ephemeral=True)
    logging.info(f"{interaction.user} used command /new.")


@bot.tree.command(name="feedback", description="Sends feedback to the developer of John Bot.")
...
def main():
    print("Welcome to John Bot v0.1.5")
